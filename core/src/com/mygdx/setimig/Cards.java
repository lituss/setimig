...
public class Cards {
    ...
    public void putCard(Card card,int posx,int posy, float angle,float temps){
        card.marcaNovaPosicio(posx, posy, angle, temps,0,true);
        putCard(card);
    }
    public void putCard(Card card,int posx,int posy, float angle,float temps,float delay) {
        card.marcaNovaPosicio(posx, posy, angle, temps, delay,true);
        putCard(card);
    }
    ...
    public void tapa(boolean tapada){
        int contador=0;for (Card card : cards) {card.tapa(tapada);contador++;}
        Gdx.app.log("sim","contador tapa : "+(contador));
    }
    public void mou(int x, int y,float delta){
        for (Card card : cards){
            card.marcaNovaPosicio(x,y, 90, delta,0,false);
            delta+=0.1;
        }
    }
}
